/*
* 题意：给出两棵树，有若干个询问，每次询问u,v,需要返回这两个节点分别在各自的树上的最近公共祖先；
*       保证父亲节点的编号一定小于儿子。强制在线。
*
* 做法： 首先考虑离线的做法，对于一个第一棵树上特定的u点，我们在线段树中保存根节点到u节点的信息，对于u节点到根上的节点x对于子树上的所有点来说都是父亲。如果将子树上面的所有点对应的第二棵树上的点都标记上父亲的标号，并且取最大标号留下的话，那么做完之后第二棵树中每个节点的标号就是这个节点与u节点的最近公共祖先(不同树上的)。那么也就是说这里对于第一棵树中每一个节点来说都得有一棵对应的线段树，那么很容易就想到了主席树的做法。每个节点的信息从父亲处继承下来。那么对于一个询问就在相应的线段树上面查询即可。
*
*
*
*/
